...
button_controlled_LED::button_controlled_LED() :
    scheduler_task("button_LED", 4 * 512, PRIORITY_HIGH)
{

}

bool button_controlled_LED::init(void)
{
    LPC_PINCON->PINSEL2 &= ~(3 << 2); //set functionality to GPIO for on-board LED
    LPC_PINCON->PINSEL2 &= ~(3 << 20); //set functionality to GPIO for on-board button
    LPC_PINCON->PINSEL3 &= ~(3 << 14); //set functionality to GPIO for periferal LED
    LPC_PINCON->PINSEL1 &= ~(3 << 28); //set functionality to GPIO for periferal button

    LPC_GPIO1->FIODIR |= (1 << 1); //sets P1.1 to output
    LPC_GPIO1->FIODIR &= ~(1 << 10); //sets P1.10 to input
    LPC_GPIO0->FIODIR &= ~(1 << 29); //sets P0.29 to input
    LPC_GPIO1->FIODIR |= (1 << 23); //sets p1.23 to output

    return true;
}

bool button_controlled_LED::run(void *p)
{
    if (LPC_GPIO1->FIOPIN & (1 << 10)) //on-board
    {
    ...
    if (LPC_GPIO0->FIOPIN & (1 << 29)) //external
    {
